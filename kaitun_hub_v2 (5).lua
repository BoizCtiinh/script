...
-- ── Bước 0: Choose Team (chạy ngay khi load, trước tất cả hệ thống khác) ──
do
    print("[Kaitun] ⏳ Chờ chọn team: " .. tostring(getgenv().Team or "Pirates") .. "...")
    repeat task.wait()
        pcall(function()
            for i, v in pairs(getconnections(
                game.Players.LocalPlayer.PlayerGui["Main (minimal)"]
                .ChooseTeam.Container[(getgenv().Team or "Pirates")]
    ...
    print("[Kaitun] ✓ Team đã chọn:", tostring(Player.Team and Player.Team.Name or "Unknown"))
end
    ...
    while task.wait(0.1) do
        ...
        --   false     → không có gì cần làm với melee
            ...
            elseif HasFruitOnMap() then
                ...
                DisableFastAttack()
                _G.CurrentTarget = nil
                _G.CollectingFruit = true
                TweenToFruits()
                _G.CollectingFruit = false
...
-- Loop 7: Auto Hop mỗi 60 phút (chỉ khi đang farm level bình thường)
-- Không hop khi: đang collect fruit, đang có target, đang làm quest đặc biệt
task.spawn(function()
    local HOP_INTERVAL = 3600  -- 60 phút tính bằng giây
    local lastHopTime  = tick()

    while task.wait(30) do
        if tick() - lastHopTime >= HOP_INTERVAL then
            -- Kiểm tra các điều kiện "đang bận"
            local isBusy = _G.CollectingFruit          -- đang collect fruit
                        or (_G.CurrentTarget ~= nil)    -- đang đánh mob cụ thể
                        or HasFruitOnMap()              -- có fruit trên map chưa collect

            if not isBusy then
                local timeMin = math.floor(HOP_INTERVAL / 60)
                SetTask("SubTask", string.format(
                    "[AutoHop] ⏱ %d phút đã trôi qua → Hop server tìm exp tốt hơn...",
                    timeMin
                ))
                lastHopTime = tick()
                HopServers()
            else
                -- Bận → trì hoãn hop, reset timer để tính lại từ khi xong việc
                lastHopTime = tick()
                SetTask("SubTask", "[AutoHop] Đang bận (collect/farm) → hoãn hop...")
            end
        end
    end
end)
