...
local function KillEnemy(enemy)
    ...
    pcall(function()
        ...
        repeat
            task.wait()
            if _meleeBuying then break end  -- nhường ngay khi AutoFullyMelee cần
...
-- =====================================================
--  PORTAL SYSTEM
--  Sea 1 portals (dùng "requestEntrance" remote):
--    Sky Arena 1    : Vector3(-4654, 872, -1759)  → exit  Sky Palace
--    Sky Arena 2    : Vector3(-7894, 5547, -380)  → enter Sky Palace
--    UnderWater 1   : Vector3(3876, 35, -1939)    → exit  UnderWater City
--    UnderWater 2   : Vector3(61163, 11, 1819)    → enter UnderWater City
-- =====================================================

local PORTAL = {
    SKY_ENTER   = Vector3.new(-7894, 5547, -380),
    SKY_EXIT    = Vector3.new(-4654, 872, -1759),
    WATER_ENTER = Vector3.new(61163, 11, 1819),
    WATER_EXIT  = Vector3.new(3876, 35, -1939),
}

-- Dùng workspace._WorldOrigin.Locations để xác định player đang ở khu vực nào
local function GetPlayerArea()
    ...
    for _, loc in pairs(locs:GetChildren()) do
        local mesh = loc:FindFirstChildOfClass("SpecialMesh")
        local radius = mesh and mesh.Scale.X or 0
        if radius > 0 and (hrp.Position - loc.Position).Magnitude <= radius then
            return loc.Name
        end
    end
    return ""
...
-- Tween đến gần portal rồi invoke requestEntrance
-- Cần tween tới portal trước khi invoke để server nhận đúng vị trí
local function PortalTeleport(portalVec)
    if not Player.Character or not Player.Character:FindFirstChild("HumanoidRootPart") then return end
    local dist = (Player.Character.HumanoidRootPart.Position - portalVec).Magnitude
    if dist > 80 then
        topos(CFrame.new(portalVec))
        WaitTween(15)
    end
    pcall(function()
        ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", portalVec)
    end)
    task.wait(2)
end

-- Kiểm tra player có đang trong Sky Palace không (Y cao hoặc tên khu vực)
local function IsInSkyPalace()
    local area = GetPlayerArea():lower()
    if area:find("sky") then return true end
    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    return hrp and hrp.Position.Y > 4000 or false
end

-- Kiểm tra player có đang trong UnderWater City không
local function IsInUnderWater()
    local area = GetPlayerArea():lower()
    if area:find("underwater") or area:find("under water") then return true end
    -- Fallback: sau khi vào portal WATER_ENTER, player ở rất xa về trục X
    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    return hrp and hrp.Position.X > 30000 or false
end

-- Level 375–449 cần farm trong UnderWater City
local function NeedsUnderWater(level)
    return level >= 375 and level <= 449
end

-- Level 475–624 cần farm trong Sky Palace
local function NeedsSkyPalace(level)
    return level >= 475 and level <= 624
end

-- Xử lý teleport vào zone và rời zone khi cần thiết.
-- Trả về true nếu đã handle (caller không cần làm gì thêm tick này).
local function HandleZoneTeleport(level)
    if NeedsUnderWater(level) then
        if not IsInUnderWater() then
            SetTask("MainTask", string.format("[Zone] Lv%d → vào UnderWater City...", level))
            SetTask("SubTask", "[Zone] Dùng portal UnderWater City 2")
            PortalTeleport(PORTAL.WATER_ENTER)
            return true
        end
        -- Trong zone → farm bình thường (không return true, để AutoFarmLevel chạy)
        return false
    end

    if NeedsSkyPalace(level) then
        if not IsInSkyPalace() then
            SetTask("MainTask", string.format("[Zone] Lv%d → vào Sky Palace...", level))
            SetTask("SubTask", "[Zone] Dùng portal Sky Arena 2")
            PortalTeleport(PORTAL.SKY_ENTER)
            return true
        end
        return false
    end

    -- Nếu đang trong zone nhưng level đã thoát khỏi range → thoát zone
    if IsInUnderWater() then
        SetTask("SubTask", "[Zone] Thoát UnderWater City → portal 1")
        PortalTeleport(PORTAL.WATER_EXIT)
        return true
    end
    if IsInSkyPalace() then
        SetTask("SubTask", "[Zone] Thoát Sky Palace → portal 1")
        PortalTeleport(PORTAL.SKY_EXIT)
        return true
    end
...
local function SkipModeFarm(level)
    ...
    if enemy then
        ...
        if nextEnemy then KillEnemy(nextEnemy) end
    end
    return true
end
...
local function AutoFarmLevel()
    -- Guard: nếu đang đi mua melee thì không làm gì
    if _meleeBuying then return end

    local ok, level = pcall(function() return Player.Data.Level.Value end)
    if not ok or not level then task.wait(1); return end

    -- Xử lý teleport zone (UnderWater / Sky Palace) trước khi farm
    if HandleZoneTeleport(level) then return end
    ...
    pcall(function()
        ...
        if not Player.PlayerGui.Main.Quest.Visible then
            -- Guard trước khi tween tới NPC
            if _meleeBuying then return end
            SetTask("SubTask", string.format("[AutoFarm] Tween → NPC: %s", Qname))
            topos(PosQ); WaitTween(12)
            if _meleeBuying then return end  -- check lại sau khi tween xong
            if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if Workspace.Enemies:FindFirstChild(Mon) then
                ...
                for _, v in pairs(Workspace.Enemies:GetChildren()) do
                    if _meleeBuying then break end  -- dừng vòng lặp mob ngay
                    if v.Name == Mon
                    ...
                    then
                        ...
                        repeat
                            task.wait()
                            if _meleeBuying then break end  -- thoát farm loop ngay
                ...
                if _meleeBuying then return end  -- không tween spawn point nếu đang mua melee
