...
-- SetTask fallback (sẽ bị override bởi Task System sau)
if not SetTask then
    function SetTask(key, msg) print("[" .. key .. "] " .. msg) end
end

-- =====================================================
--  SEA DETECTION (by PlaceId — chính xác hơn MAP attr)
-- =====================================================
...
-- Fallback: nếu PlaceId không khớp thì dùng workspace MAP attribute
if not Sea1 and not Sea2 and not Sea3 then
    local rz_MAP = workspace:GetAttribute("MAP")
    Sea1 = rz_MAP == "Sea1"
    Sea2 = rz_MAP == "Sea2" or rz_MAP == "Dungeons"
    Sea3 = rz_MAP == "Sea3"
end

local World = Sea1 and 1 or Sea2 and 2 or Sea3 and 3 or 1
print(string.format("[Kaitun] Sea Detection → Sea%d (PlaceId: %d)", World, game.PlaceId))

-- =====================================================
--  MELEE JSON PERSISTENCE
--  Lưu/đọc mastery melee vào file: Username-Kaitun.json
--  Format: {"Black Leg": 400, "Electro": 200, ...}
-- =====================================================

local MELEE_SAVE_FILE = Player.Name .. "-Kaitun.json"

-- MeleeCache: table sống trong RAM, đồng bộ với file JSON
-- key = tên melee (string), value = mastery (number)
local MeleeCache = {}

local function SaveMeleeCache()
    pcall(function()
        writefile(MELEE_SAVE_FILE, HttpService:JSONEncode(MeleeCache))
    end)
end

local function LoadMeleeCache()
    local ok, data = pcall(function()
        if isfile(MELEE_SAVE_FILE) then
            return HttpService:JSONDecode(readfile(MELEE_SAVE_FILE))
        end
        return {}
    end)
    if ok and type(data) == "table" then
        MeleeCache = data
        print("[Kaitun] ✓ Đọc melee data từ " .. MELEE_SAVE_FILE)
        for name, mastery in pairs(MeleeCache) do
            print(string.format("  → %s: %d", name, mastery))
        end
    else
        MeleeCache = {}
    end
...
-- Cập nhật MeleeCache từ tool đang cầm/backpack rồi save
local function UpdateMeleeCacheFromTool(tool)
    if not tool or not tool:IsA("Tool") then return end
    if tool.ToolTip == "Melee" or tool:GetAttribute("WeaponType") == "Melee" then
        local levelVal = tool:FindFirstChild("Level")
        if levelVal then
            local prev = MeleeCache[tool.Name] or 0
            MeleeCache[tool.Name] = math.max(prev, levelVal.Value)
            SaveMeleeCache()
        end
    end
end

-- Quét backpack + character hiện tại để load mastery mới nhất
local function SyncMeleeCacheFromCharacter()
    for _, container in pairs({Player.Backpack, Player.Character}) do
        if container then
            for _, tool in pairs(container:GetChildren()) do
                UpdateMeleeCacheFromTool(tool)
            end
        end
    end
end

-- LocalStorage giữ lại cho code cũ (redeem codes)
local LocalStorage = {}
...
-- =====================================================
--  BYPASS TELEPORT (ported from BF-MainNightHub)
--  Teleport nhanh qua spawn point gần nhất nếu đích quá xa
-- =====================================================

-- Tìm spawn point gần một vị trí (radius 2500 studs)
local function GetSpawnPoint(pos)
    if typeof(pos) == "CFrame" then pos = pos.Position end
    local ok, result = pcall(function()
        for _, v in pairs(workspace._WorldOrigin.PlayerSpawns.Pirates:GetChildren()) do
            if (v.Part.Position - pos).Magnitude <= 2500 then
                return v
            end
        end
    end)
...
-- Tìm spawn point tốt nhất để làm "bàn đạp" đến target
local function GetBypassCFrame(targetCFrame)
    local targetPos = typeof(targetCFrame) == "CFrame" and targetCFrame.Position or targetCFrame
    if not Player.Character or not Player.Character:FindFirstChild("HumanoidRootPart") then return nil end
    local hrpPos = Player.Character.HumanoidRootPart.Position
    local Max, Pos = math.huge, nil
    local ok = pcall(function()
        for _, v in pairs(workspace._WorldOrigin.PlayerSpawns.Pirates:GetChildren()) do
            local distTarget = (v.Part.Position - targetPos).Magnitude
            local distPlayer = (hrpPos - targetPos).Magnitude
            local distSpawnPlayer = (v.Part.Position - hrpPos).Magnitude
            if distPlayer >= 3000
                and GetSpawnPoint(v.Part.Position) ~= GetSpawnPoint(hrpPos)
                and distSpawnPlayer <= 10000
                and distTarget <= Max
            then
                Max = distTarget
                Pos = v
...
-- Kiểm tra có thể bypass teleport không
local function CanBypassTeleport(targetCFrame)
    local targetPos = typeof(targetCFrame) == "CFrame" and targetCFrame.Position or targetCFrame
    if not Player.Character or not Player.Character:FindFirstChild("HumanoidRootPart") then return false end
    -- Quá gần thì không cần bypass
    if (Player.Character.HumanoidRootPart.Position - targetPos).Magnitude <= 3500 then return false end
    -- Kiểm tra area đặc biệt
    local areaName = InArea(targetPos).Name
    if areaName == "" then return false end
    if areaName:find("Dimension") or areaName:find("Submerged")
        or areaName == "Sealed Cavern" or areaName:lower():find("under") then
        return false
    end
    -- Có legendary item trong tay thì không bypass (tránh mất item)
    local function hasItem(name)
        return Player.Backpack:FindFirstChild(name)
            or (Player.Character and Player.Character:FindFirstChild(name))
    end
    if hasItem("God's Chalice") or hasItem("Fist of Darkness")
        or hasItem("Sweet Chalice") or hasItem("Hallow Essence") then
        return false
    end
    -- Đang ở SubmergedIsland thì không bypass
    local ok, lastSpawn = pcall(function() return Player.Data.LastSpawnPoint.Value end)
    if ok and lastSpawn == "SubmergedIsland" then return false end
    return true
end

local _lastBypassTick = 0
-- Thực hiện bypass teleport đến target
-- Trả về true nếu thành công
local function BypassTP(targetCFrame)
    if tick() - _lastBypassTick < 8 then return false end
    if not CanBypassTeleport(targetCFrame) then return false end
    local bypass = GetBypassCFrame(targetCFrame)
    if not bypass then return false end
    local ok = pcall(function()
        Player.Character.LastSpawnPoint.Disabled = true
        ReplicatedStorage.Remotes.CommF_:InvokeServer("SetLastSpawnPoint", bypass.Name)
        ReplicatedStorage.Remotes.CommF_:InvokeServer("SetSpawnPoint")
        Player.Character:PivotTo(bypass.Part.CFrame)
    ...
    if ok then _lastBypassTick = tick() end
    return ok
end

-- =====================================================
--  AUTO TRAVEL DRESSROSA (ported from BF-MainNightHub)
--  Chỉ chạy ở Sea1, level >= 700
-- =====================================================

-- Lấy bartilo plate chưa được kích hoạt (màu Sand yellow)
local function GetBartiloPlates()
    local ok, result = pcall(function()
...
-- Kiểm tra nên chạy Auto Travel Dressrosa không
-- Trả về progress step (string) hoặc false
local function CheckTravelDressrosa()
    if not Sea1 then return false end
    local ok, level = pcall(function() return Player.Data.Level.Value end)
    if not ok or level < 700 then return false end
    -- Kiểm tra đã completed chưa (progress == 0 nghĩa là chưa bắt đầu)
    local ok2, prog = pcall(function()
        return ReplicatedStorage.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa")
    end)
    if ok2 and prog == 0 then return "travel_ready" end
    -- Kiểm tra Ice Door còn khoá không
    local ok3, doorLocked = pcall(function() return workspace.Map.Ice.Door.CanCollide end)
    if ok3 and doorLocked then return "need_key" end
    -- Kiểm tra Ice Admiral còn sống không (cần giết để mở cửa)
    if ok3 and not doorLocked then
        local iceAdmiral = GetMobByName("Ice Admiral")
        if iceAdmiral then return "kill_admiral" end
        return "wait_admiral"
    end
    return false
end

-- Thực thi một bước của Auto Travel Dressrosa
local function DoTravelDressrosa(step)
    if step == "travel_ready" then
        SetTask("SubTask", "[Sea2] Đang teleport tới Dressrosa...")
        task.wait(0.5)
        ReplicatedStorage.Remotes.CommF_:InvokeServer("TravelDressrosa")

    elseif step == "need_key" then
        if not CheckBackpack("Key") then
            SetTask("SubTask", "[Sea2] Lấy key từ Detective...")
            ReplicatedStorage.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Detective")
        else
            SetTask("SubTask", "[Sea2] Dùng Key mở cửa Ice Castle...")
            EquipWeapon("Key")
            task.wait(0.2)
            pcall(function()
                firetouchinterest(Player.Character.Key.Handle, workspace.Map.Ice.Door, 0)
                firetouchinterest(Player.Character.Key.Handle, workspace.Map.Ice.Door, 1)
            end)
        end

    elseif step == "kill_admiral" then
        SetTask("SubTask", "[Sea2] Giết Ice Admiral...")
        DisableFastAttack()
        EnableFastAttack()
        local v = GetMobByName("Ice Admiral")
        if v and IsAlive(v) then
            repeat
                ...
                BringEnemy(v.HumanoidRootPart.Position, {"Ice Admiral"})
            until not IsAlive(v)
            DisableFastAttack()
        end

    elseif step == "wait_admiral" then
        SetTask("SubTask", "[Sea2] Chờ Ice Admiral spawn...")
...
-- Lấy mastery hiện tại của melee
-- Ưu tiên: tool đang cầm (realtime) → MeleeCache (từ file) → nil (chưa có)
local function GetMeleeMastery(meleeName)
    -- Kiểm tra trực tiếp backpack/character (realtime, chính xác nhất)
    for _, container in pairs({Player.Backpack, Player.Character}) do
        if container then
            for _, tool in pairs(container:GetChildren()) do
                if tool:IsA("Tool") and tool.Name == meleeName then
                    local levelObj = tool:FindFirstChild("Level")
                    local mastery = levelObj and levelObj.Value or 0
                    -- Cập nhật cache nếu mastery mới cao hơn
                    if not MeleeCache[meleeName] or mastery > MeleeCache[meleeName] then
                        MeleeCache[meleeName] = mastery
                        SaveMeleeCache()
                    end
                    return mastery
                end
    ...
    -- Không thấy trong backpack/character → tra cache (có thể đã mua nhưng không equip)
    if MeleeCache[meleeName] then
        return MeleeCache[meleeName]
    end
...
local function BuyMeleeFromNPC(meleeData)
    ...
    -- Kiểm tra đã có chưa + cập nhật cache
    if CheckBackpack(meleeData.name) then
        SetTask("SubTask", "[AutoMelee] ✓ Đã mua thành công: " .. meleeData.name)
        -- Ghi vào MeleeCache với mastery = 0 (mới mua)
        if not MeleeCache[meleeData.name] then
            MeleeCache[meleeData.name] = 0
            SaveMeleeCache()
            print("[MeleeCache] Saved: " .. meleeData.name .. " = 0 → " .. MELEE_SAVE_FILE)
        end
...
-- Trạng thái global cho Task System: true khi tất cả melee hoàn thành
local _meleeAllDone = false

-- =====================================================
--  TASK SYSTEM / STATE MACHINE
--  Inspired by AryaMain's TasksOrder + Refresh/Start pattern
--
--  Cơ chế:
--    • Mỗi task có Refresh() → trả về data (truthy) nếu cần chạy, false nếu không
--    • Mỗi task có Start(data) → thực thi 1 action, không loop
--    • RunTasks() duyệt TasksOrder theo priority, task đầu tiên Refresh() truthy
--      sẽ được Start() gọi, rồi return (vòng lặp tiếp theo bắt đầu lại từ đầu)
--    • Ưu tiên cao → index nhỏ trong TasksOrder
-- =====================================================

local TaskRegistry = {} -- { [taskName] = { Refresh=fn, Start=fn } }
local TasksOrder   = {} -- array of task names, index 1 = highest priority
local CurrentTaskName = ""

local function RegisterTask(name, refreshFn, startFn)
    TaskRegistry[name] = { Refresh = refreshFn, Start = startFn }
    table.insert(TasksOrder, name)
end

-- Chạy một vòng task loop: duyệt priority, gọi task đầu tiên phù hợp
local function RunTasks()
    for _, taskName in ipairs(TasksOrder) do
        local taskDef = TaskRegistry[taskName]
        if taskDef then
            local okR, data = pcall(taskDef.Refresh)
            if okR and data then
                if CurrentTaskName ~= taskName then
                    CurrentTaskName = taskName
                    print("[TaskSystem] ► Active task: " .. taskName)
                end
                local okS, err = pcall(taskDef.Start, data)
                if not okS then
                    warn("[TaskSystem] Error in '" .. taskName .. "': " .. tostring(err))
                end
                return -- chỉ chạy 1 task mỗi tick
            end
        end
    end
end
...
-- ───────────────────────────────────────────────────
--  ĐĂNG KÝ CÁC TASK (priority cao nhất ở trên)
-- ───────────────────────────────────────────────────

-- TASK 1: Auto Travel Dressrosa (Sea1, lv ≥ 700)
RegisterTask("AutoTravelDressrosa",
    function() -- Refresh
        return CheckTravelDressrosa()
    end,
    function(step) -- Start
        SetTask("MainTask", "[AutoTravelDressrosa] Chuẩn bị vào Sea 2...")
        DoTravelDressrosa(step)
    end
)

-- TASK 2: Auto Saber (lv ≥ 300, chưa có saber)
RegisterTask("AutoSaber",
    function() -- Refresh
        local ok, level = pcall(function() return Player.Data.Level.Value end)
        if not ok or level < 300 then return false end
        if _saberDone then return false end
        if CheckItem("Saber") then _saberDone = true; return false end
        return true
    end,
    function(_) -- Start
        SetTask("MainTask", "[AutoSaber] Đang làm quest Saber...")
        local ok, err = pcall(GetSaber)
        if not ok then
            SetTask("SubTask", "[AutoSaber] Lỗi: " .. tostring(err))
            task.wait(2)
        end
    end
)

-- TASK 3: Auto Melee - Buy (cần mua melee mới)
RegisterTask("AutoMeleeBuy",
    function() -- Refresh
        if _meleeAllDone then return false end
        for _, meleeData in ipairs(MeleesOrder) do
            if meleeData.requireSea and meleeData.requireSea > World then break end
            if meleeData.requireSea and meleeData.requireSea ~= World then continue end
            if meleeData.extraCheck and not meleeData.extraCheck() then break end
            local mastery = GetMeleeMastery(meleeData.name)
            if mastery == nil then
                local canBuy = CanAffordMelee(meleeData.price)
                if canBuy then return meleeData end -- có tiền → cần mua
                -- Không đủ tiền → thông báo và tiếp tục farm
                local missingParts = {}
                for cur, amt in pairs(meleeData.price) do
                    local diff = amt - GetCurrency(cur)
                    if diff > 0 then
                        table.insert(missingParts, cur .. " +" .. diff)
                    end
                end
                SetTask("SubTask", "[AutoMelee] Chờ đủ tiền cho " .. meleeData.name
                    .. " | " .. table.concat(missingParts, ", "))
                break -- farm kiếm tiền
            end
        end
        return false
    end,
    function(meleeData) -- Start
        SetTask("MainTask", "[AutoMelee] ► Mua " .. meleeData.name)
        BuyMeleeFromNPC(meleeData)
    end
)

-- TASK 4: Auto Melee - Mastery (có melee rồi nhưng mastery chưa đủ)
RegisterTask("AutoMeleeMastery",
    function() -- Refresh
        if _meleeAllDone then return false end
        local allDone = true
        for _, meleeData in ipairs(MeleesOrder) do
            if meleeData.requireSea and meleeData.requireSea > World then break end
            if meleeData.requireSea and meleeData.requireSea ~= World then continue end
            if meleeData.extraCheck and not meleeData.extraCheck() then allDone = false; break end
            local mastery = GetMeleeMastery(meleeData.name)
            if mastery == nil then
                allDone = false -- chưa mua → Task 3 xử lý
                break
            elseif mastery < meleeData.mastery then
                allDone = false
                return meleeData -- cần farm mastery
            end
        end
        if allDone then _meleeAllDone = true end
        return false
    end,
    function(meleeData) -- Start
        SetTask("SubTask", string.format("[AutoMelee] Farm mastery %s: %d/%d",
            meleeData.name, GetMeleeMastery(meleeData.name) or 0, meleeData.mastery))
        EquipMelee(meleeData.name)
        -- Farming tiếp tục bởi AutoFarm bên dưới (task này chỉ equip đúng melee)
    end
)

-- TASK 5: Collect Fruit trên map
RegisterTask("CollectFruit",
    function() -- Refresh
        return HasFruitOnMap() and true or false
    end,
    function(_) -- Start
        SetTask("MainTask", "[Fruit] Thu thập fruit trên map...")
        DisableFastAttack()
        _G.CurrentTarget = nil
        TweenToFruits()
    end
...
-- TASK 6: Auto Farm Level (luôn chạy nếu không có task nào khác)
RegisterTask("AutoFarm",
    function() -- Refresh
        return true -- luôn sẵn sàng
    end,
    function(_) -- Start
        local ok, level = pcall(function() return Player.Data.Level.Value end)
        SetTask("MainTask", string.format("[AutoFarm] Lv%d | Đang farm...", ok and level or 0))
...
-- ── Bước 1: Load Melee Cache từ file JSON ──────────
LoadMeleeCache()
SyncMeleeCacheFromCharacter()

-- ── Bước 2: Theo dõi mastery melee realtime ────────
task.spawn(function()
    -- Listener cho character (tool equipped)
    local function watchCharacter(char)
        if not char then return end
        char.ChildAdded:Connect(function(item)
            task.wait(0.1)
            UpdateMeleeCacheFromTool(item)
        end)
        -- Level changed listener cho melee đang equip
        for _, tool in pairs(char:GetChildren()) do
            if tool:IsA("Tool") and tool.ToolTip == "Melee" then
                local levelObj = tool:FindFirstChild("Level")
                if levelObj then
                    levelObj.Changed:Connect(function(newVal)
                        local prev = MeleeCache[tool.Name] or 0
                        if newVal > prev then
                            MeleeCache[tool.Name] = newVal
                            SaveMeleeCache()
                            print(string.format("[MeleeCache] %s mastery → %d", tool.Name, newVal))
                        end
                    end)
                end
            end
    ...
    -- Khi equip tool mới thì check xem có phải melee không
    Player.Character.ChildAdded:Connect(function(item)
        task.wait(0.2)
        if item:IsA("Tool") and (item.ToolTip == "Melee" or item:FindFirstChild("Level")) then
            UpdateMeleeCacheFromTool(item)
            -- Hook level changed
            local levelObj = item:FindFirstChild("Level")
            if levelObj then
                levelObj.Changed:Connect(function(newVal)
                    local prev = MeleeCache[item.Name] or 0
                    if newVal > prev then
                        MeleeCache[item.Name] = newVal
                        SaveMeleeCache()
                        print(string.format("[MeleeCache] %s mastery → %d", item.Name, newVal))
                    end
                end)
            end
        end
    ...
    -- Theo dõi backpack (khi tool về backpack)
    Player.Backpack.ChildAdded:Connect(function(item)
        task.wait(0.1)
        UpdateMeleeCacheFromTool(item)
    end)

    -- Re-register khi respawn
    ...
    print("[Kaitun] ✓ MeleeCache watcher → Online | File: " .. MELEE_SAVE_FILE)
end)

-- ── Bước 3: MAIN TASK LOOP (State Machine) ─────────
-- Duyệt TasksOrder mỗi tick, chạy task ưu tiên cao nhất
task.spawn(function()
    while task.wait(0.1) do
        local ok, err = pcall(RunTasks)
        if not ok then
            warn("[TaskSystem] Unhandled error: " .. tostring(err))
            task.wait(1)
        end
    end
end)

-- ── Bước 4: Auto Stats (event-driven) ──────────────
task.spawn(function()
...
-- ── Bước 5: Auto Haki (giữ Buso luôn bật, 5s/lần) ─
task.spawn(function()
    while task.wait(5) do AutoHaki() end
end)

-- ── Bước 6: Redeem Code X2 EXP ─────────────────────
task.spawn(function()
    task.wait(3)
    RedeemCodes()
...
print(string.format("[Kaitun] ✓ Loaded | Sea%d | Tasks: %d registered | BringMob=%s",
    World, #TasksOrder, tostring(_G.BringMob)))
for i, name in ipairs(TasksOrder) do
