...
-- =====================================================
--  CHOOSE TEAM (chạy ngay khi script load, trước mọi thứ)
-- =====================================================
repeat task.wait()
    pcall(function()
        for _, v in pairs(getconnections(
            Player.PlayerGui["Main (minimal)"].ChooseTeam
                .Container[(getgenv().Team or "Pirates")]
...
print("[Kaitun] ✓ Team đã chọn:", tostring(Players.LocalPlayer.Team))
...
function EnableFastAttack()
    ...
    AttackConnection = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            -- Fire RIÊNG từng mob thay vì batch tất cả vào 1 lần fire
            -- Server có cap số hit/fire → batch sẽ chỉ process 2 đầu tiên
            for _, enemy in pairs(hits) do
                if not enemy or not enemy.Parent then continue end
                if not enemy:FindFirstChild("Humanoid") or enemy.Humanoid.Health <= 0 then continue end
                if not enemy:FindFirstChild("HumanoidRootPart") then continue end
                RegisterAttack:FireServer(0)
                RegisterHit:FireServer(
                    enemy.Head,
                    {{[1] = enemy, [2] = enemy.HumanoidRootPart}, enemy},
                    nil,
                    "078da5141"
                )
            end
...
-- Rate-limit bring: tối đa 1 lần / 0.5s để tránh FPS drop
local _lastBringTick = 0
local BRING_INTERVAL = 0.5

-- targetNames: table tên mob được phép bring (nil = bring tất cả)
-- PosMon     : vị trí đích để tập hợp mob về (thường là vị trí player)
local function BringEnemy(PosMon, targetNames)
    if not _G.BringMob then return end
    if tick() - _lastBringTick < BRING_INTERVAL then return end
    _lastBringTick = tick()
    ...
    -- Dùng vị trí player làm tâm đo khoảng cách (không phải PosMon)
    -- → bring được mob dù chúng ở xa nhau
    ...
    pcall(function()
        for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
            -- Bỏ qua nếu không đúng loại mob
            if targetNames and not nameSet[enemy.Name] then continue end

            -- Ghost check: phải tồn tại đúng trong Enemies folder
            ...
            -- Đo khoảng cách tới PLAYER (không phải PosMon)
            -- → mob ở bất kỳ đâu trong vùng 500 studs đều được bring
            local distToPlayer = (hrp.Position - playerPos).Magnitude
            if distToPlayer > 20 and distToPlayer <= 500 then
                local offset = Vector3.new(
                    math.random(-6, 6),
                    0,
                    math.random(-6, 6)
                )
                hrp.CFrame = CFrame.new(PosMon + offset)
                didBring = true
            end
        end
    end)

    -- SimulationRadius: chỉ set 1 lần/chu kỳ, reset về normal sau khi bring
    if didBring then
...
local function HopServers()
    SetTask("SubTask", "[Hop] Hop server sau 5 giây...")
    task.wait(5)
    pcall(function()
        local servers = {}
...
local _lastHopTime  = tick()  -- reset mỗi khi còn task quan trọng, hop khi đủ 60 phút
...
-- Loop 7: Auto Hop mỗi 60 phút — chỉ hop khi KHÔNG còn việc gì quan trọng
-- (tránh interrupt Saber / Sea2 / Bartilo / mua melee đang dở)
task.spawn(function()
    local HOP_INTERVAL = 60 * 60  -- 60 phút tính bằng giây

    while task.wait(30) do  -- kiểm tra mỗi 30s
        -- Điều kiện "đang farm thuần": tất cả quest/task lớn đã xong hoặc chưa đến level
        local ok, level = pcall(function() return Player.Data.Level.Value end)
        local lvl = ok and level or 0

        local saberPending    = (lvl >= 300) and not _saberDone
        local sea2Pending     = (lvl >= 700) and not _sea2Done  and World < 2
        local bartiloPending  = (lvl >= 850) and not _bartiloQuestDone and World == 2
        local meleeBuying     = not _meleeAllDone  -- conservative: skip hop jika melee belum done

        if saberPending or sea2Pending or bartiloPending or meleeBuying then
            -- Còn việc quan trọng → reset timer, không hop
            _lastHopTime = tick()
        elseif tick() - _lastHopTime >= HOP_INTERVAL then
            SetTask("MainTask", "[AutoHop] 60 phút trôi qua → Hop server mới...")
            DisableFastAttack()
            _G.CurrentTarget = nil
            HopServers()
        end
